			...
			<head>
				...
				for(var i=0;i<aA.length;i++){
					...
					aA[i].onclick = function(){
						...
						var template = '<div class="detailItem caseDetail stepDetail" id="stepDetail">\
					...
					<div class="close">\
						...
						var oTable = stepDetail.getElementsByTagName("table")[0];
						
						var otbody = document.createElement("tbody");
						
						var xmlDom = createXMLDom("report.xml");
						
						var testcasenode = xmlDom.getElementsByTagName("testcase")[this.num];
						
						//console.log(testcasenode)
						
						var step = testcasenode.getElementsByTagName("step");
						...
						for(var i=0;i<step.length;i++){
							...
							var steplen = step[i].children.length;
							
							//console.log(step[i].children[0])
							...
							for(j=0;j<steplen;j++){
								...
								if(step[i].children[j].children.length){
								
									for(var z=0;z<step[i].children[j].children.length;z++){
										
										if(step[i].children[j].children[z].childNodes[0]){
											str += step[i].children[j].children[z].childNodes[0].nodeValue+' ';
										}else{
											str = '';
										}
									}
									...
									str = step[i].children[j].childNodes[0].nodeValue
								}
